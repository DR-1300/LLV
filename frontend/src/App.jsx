...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (nodes.length === 0) {
      ...
      ctx.fillText("Type a value and click + Create", W / 2, H / 2);
      return;
    }
    ...
    nodes.forEach(n => {
      ...
      // Value label — Minecraft-style monospace font
      ctx.fillStyle = isHighlighted || isPending ? ESP : PEO;
  ...
  function drawArrow(ctx, x1, y1, x2, y2, highlight) {
    ...
    const ex = x2 - ux * (NODE_R + 5), ey = y2 - uy * (NODE_R + 5);
    ctx.beginPath();
    ctx.moveTo(sx, sy);
    ctx.lineTo(ex, ey);
  ...
  function handleMouseMove(e) {
    ...
    moveNode(
      dragging.current,
      Math.max(NODE_R + 10, Math.min(rect.width - NODE_R - 10, x - dragOffset.current.x)),
      Math.max(NODE_R + 10, Math.min(rect.height - NODE_R - 10, y - dragOffset.current.y))
    );
  }
  ...
  function handleClick(e) {
    if (connectingFrom !== null) {
      const { x, y } = canvasXY(e);
      const n = nodeAt(x, y);
      if (n && n.id !== connectingFrom) connect(connectingFrom, n.id);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: PEO_L }}>
      ...
      <div style={{ background: ESP, color: PEO, padding: "9px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <span style={{ fontSize: 14, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", fontFamily: "'Courier New', monospace" }}>Linked List Visualiser</span>
      ...
      <div style={{ background: ESP, borderTop: "1px solid #5a3530", padding: "7px 18px", display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap", flexShrink: 0 }}>
        <input
          ...
          placeholder="value…"
          maxLength={6}
          style={{ background: "rgba(244,201,214,.1)", border: "1px solid rgba(244,201,214,.28)", color: PEO, fontSize: 12, padding: "4px 9px", borderRadius: 5, width: 80, outline: "none", fontFamily: "'Courier New', monospace" }}
        />
        <button onClick={handleAction} disabled={loading}
          style={{ background: PEO, color: ESP, border: "none", fontSize: 11, fontWeight: 700, padding: "5px 14px", borderRadius: 5, cursor: "pointer", letterSpacing: ".05em", textTransform: "uppercase", fontFamily: "'Courier New', monospace" }}>
          {loading ? "…" : actionLabel}
        </button>
        <div style={{ width: 1, height: 18, background: "rgba(244,201,214,.18)", margin: "0 2px" }} />
        {["insert", "search", "delete", "alter"].map(m => (
          <button key={m} onClick={() => { setMode(mode === m ? "create" : m); clearHighlighted(); }}
            style={{ background: mode === m ? "rgba(244,201,214,.15)" : "transparent", border: `1px solid ${mode === m ? PEO : "rgba(244,201,214,.3)"}`, color: PEO, fontSize: 11, fontWeight: 600, padding: "5px 12px", borderRadius: 5, cursor: "pointer", letterSpacing: ".05em", textTransform: "uppercase", fontFamily: "'Courier New', monospace" }}>
            {m.charAt(0).toUpperCase() + m.slice(1)}
          </button>
        ))}
        <div style={{ width: 1, height: 18, background: "rgba(244,201,214,.18)", margin: "0 2px" }} />
        <button onClick={clearAll}
          style={{ background: "transparent", border: "1px solid rgba(244,201,214,.3)", color: PEO_D, fontSize: 11, fontWeight: 600, padding: "5px 12px", borderRadius: 5, cursor: "pointer", letterSpacing: ".05em", textTransform: "uppercase", fontFamily: "'Courier New', monospace" }}>
          Clear
        </button>
      </div>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{ width: 280, background: ESP, display: "flex", flexDirection: "column", borderLeft: "1px solid rgba(244,201,214,.15)", flexShrink: 0 }}>
          <div style={{ fontSize: 10, letterSpacing: ".07em", textTransform: "uppercase", color: "rgba(244,201,214,.4)", padding: "10px 14px 2px", fontFamily: "'Courier New', monospace" }}>Current Operation</div>
          <div style={{ fontSize: 13, fontWeight: 600, color: PEO, padding: "2px 14px 10px", borderBottom: "1px solid rgba(244,201,214,.1)", fontFamily: "'Courier New', monospace" }}>
      ...
      {/* Status bar */}
      <div style={{ background: "#2a1a18", color: "rgba(244,201,214,.5)", fontSize: 10, padding: "4px 16px", flexShrink: 0, display: "flex", gap: 12, fontFamily: "'Courier New', monospace" }}>
        <span>Mode: <b style={{ color: PEO }}>{mode}</b></span>
        <span>Nodes: <b style={{ color: PEO }}>{nodes.length}</b></span>
        {pendingInsert && <span style={{ color: PEO_D }}>Select a node to insert after</span>}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            {[
              ...
              <button key={i} onClick={fn}
                style={{ display: "block", width: "100%", background: "none", border: "none", color: i === 3 ? PEO_D : PEO, fontSize: 12, padding: "8px 14px", textAlign: "left", cursor: "pointer", fontFamily: "'Courier New', monospace" }}>
                {label}
              </button>
            ))}
          </div>
      ...
      {/* Error toast — top right, auto-dismisses after 3s */}
      {error && (
        <div style={{
          position: "fixed", top: 20, right: 20, zIndex: 200,
          background: ESP, color: PEO,
          padding: "14px 20px", borderRadius: 10,
          border: "1px solid rgba(244,201,214,0.3)",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            <span>✗</span>
            <span style={{ fontWeight: 700, letterSpacing: ".04em", textTransform: "uppercase", fontSize: 11 }}>Not Found</span>
          </div>
          <div style={{ color: PEO_D, fontSize: 12 }}>{error}</div>
          <div style={{ marginTop: 8, fontSize: 10, color: "rgba(244,201,214,0.35)" }}>click to dismiss</div>
        </div>
      )}
    </div>
  );
